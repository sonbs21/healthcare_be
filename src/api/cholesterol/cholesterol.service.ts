...
export class CholesterolService {
  ...
  async getCholesterol(memberId: string, dto: FilterCholesterolGetMemberDto, pagination: Pagination) {
    try {
      const { skip, take } = pagination;
  ...
  async getCholesterolForDoctor(dto: FilterHealthRecordDto, pagination: Pagination) {
    try {
      const { skip, take } = pagination;
      const healthRecord = await this.prismaService.healthRecord.findFirst({
